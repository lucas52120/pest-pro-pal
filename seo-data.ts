/* ══════════════════════════════════════════════
   SEO DATA — Pure data module (no React imports)
   Shared between vite.config.ts (SSG) and
   src/data/cityServices.ts (runtime).
   ══════════════════════════════════════════════ */

export function deptSeo(department: string): string {
  return department.replace("(", "").replace(")", "").trim();
}

export function deptSeoPrep(department: string): string {
...
export interface ServiceDefinitionData {
  slug: string;
  title: string;
  getContent: (city: string, department: string, variation?: number) => ServiceContent;
}

export const serviceDefinitionsData: ServiceDefinitionData[] = [
  /* ═══ GUÊPES & FRELONS ═══ */
  {
    slug: "guepes-frelons",
  ...
  /* ═══ DÉSINSECTISATION ═══ */
  {
    ...
    getContent: (city: string, department: string, variation = 0) => {
      ...
      const intros = [
        `Une infestation d'insectes à ${city} exige une réponse rapide et professionnelle. G&F Nuisibles, expert en désinsectisation en ${department}, élimine cafards, punaises de lit, blattes et autres parasites. Ces envahisseurs se reproduisent à grande vitesse et constituent un risque sanitaire majeur pour votre foyer ou vos locaux à ${city}. Notre diagnostic précis permet un traitement ciblé et efficace.`,
        `La présence d'insectes indésirables dans votre logement ou commerce à ${city} compromet votre hygiène et votre tranquillité. G&F Nuisibles en ${department} met en œuvre des solutions d'éradication professionnelles contre les cafards, blattes, punaises de lit et autres parasites. Nos techniciens certifiés assurent une suppression complète et durable à ${city}.`,
        `G&F Nuisibles à ${city} vous protège contre les invasions d'insectes nuisibles en ${department}. Que vous soyez confronté à des cafards, des punaises de lit ou des blattes, nos traitements professionnels garantissent une élimination définitive. Notre expertise terrain à ${city} nous permet d'identifier rapidement le type d'infestation et d'appliquer le protocole le plus adapté.`,
        `À ${city}, en ${department}, les infestations d'insectes peuvent survenir à tout moment de l'année. G&F Nuisibles vous apporte une assistance technique spécialisée pour neutraliser cafards, punaises de lit, blattes et autres nuisibles. Notre approche combine diagnostic approfondi et traitements professionnels homologués pour une élimination totale à ${city}.`,
      ];
      ...
      const expertises = [
        ...
        `G&F Nuisibles déploie à ${city} des protocoles de désinsectisation conformes aux normes Certibiocide. En ${department}, nos techniciens maîtrisent les techniques de gel, pulvérisation et nébulisation pour une éradication complète. Chaque traitement est adapté à l'espèce identifiée et à l'environnement traité : habitation, restaurant, hôtel ou collectivité.`,
        `Notre expertise en ${department} nous permet d'intervenir à ${city} sur tous types d'infestations. Nous utilisons des biocides professionnels de dernière génération, sans danger pour les occupants et les animaux domestiques. G&F Nuisibles assure un suivi rigoureux après chaque opération pour garantir l'absence de récidive.`,
        `Les techniciens G&F Nuisibles à ${city} sont formés aux dernières méthodes d'élimination des insectes nuisibles. En ${department}, nous adaptons chaque traitement à la configuration des lieux et au degré d'infestation. Notre approche respecte les normes sanitaires les plus strictes tout en assurant une suppression définitive des parasites.`,
      ];
      ...
      const methods = [
        ...
        `À ${city}, G&F Nuisibles procède en trois phases : diagnostic terrain, traitement professionnel multi-technique et suivi de contrôle. Nos interventions en ${department} s'accompagnent systématiquement de recommandations personnalisées pour prévenir toute réinfestation future.`,
      ];
  ...
  /* ═══ DÉRATISATION ═══ */
  {
    ...
    getContent: (city: string, department: string, variation = 0) => {
      ...
      const intros = [
        `Des bruits de grattement dans vos murs à ${city} ? G&F Nuisibles intervient en urgence en ${department} pour l'éradication de rats, souris et rongeurs. Ces nuisibles provoquent des dégâts considérables : câbles électriques rongés, isolation détruite, risques d'incendie et contamination alimentaire. Ne laissez pas la situation s'aggraver à ${city}.`,
        `La présence de rongeurs à ${city} est un problème sanitaire qui nécessite une intervention professionnelle. G&F Nuisibles, spécialiste de la dératisation en ${department}, assure la suppression définitive de rats et souris dans votre habitation ou vos locaux. Nos méthodes sécurisées protègent votre famille et vos animaux domestiques à ${city}.`,
        `G&F Nuisibles à ${city} met son expertise technique au service de l'élimination des rongeurs en ${department}. Rats, souris, mulots : nous identifions précisément l'espèce et les points d'accès pour un traitement ciblé et durable. Notre connaissance du terrain à ${city} nous permet de proposer des solutions adaptées à chaque configuration.`,
        `À ${city}, en ${department}, les invasions de rongeurs touchent aussi bien les particuliers que les professionnels. G&F Nuisibles vous propose une assistance technique complète : diagnostic, neutralisation et prévention. Nos techniciens certifiés assurent un passage rapide et discret pour éliminer rats et souris de vos locaux à ${city}.`,
      ];

      const expertises = [
        `À ${city} en ${department}, nos techniciens utilisent des méthodes sécurisées, adaptées aux foyers avec enfants et animaux domestiques. Nous identifions les points d'entrée et mettons en place des solutions durables pour éviter toute réinfestation. G&F Nuisibles vous accompagne du diagnostic à la prévention.`,
        `G&F Nuisibles déploie à ${city} des dispositifs professionnels conformes aux normes en vigueur en ${department}. Postes d'appâtage sécurisés, pièges mécaniques, répulsifs ultrasoniques : nous sélectionnons la méthode la plus efficace selon l'ampleur de l'infestation et la configuration de vos locaux.`,
        ...
        `Les techniciens G&F Nuisibles à ${city} sont formés aux protocoles de dératisation les plus avancés. En ${department}, nous combinons moyens mécaniques et chimiques (appâts rodenticides sécurisés) pour une neutralisation complète. Chaque intervention est suivie d'un rebouchage des points d'accès identifiés.`,
      ];
      ...
      const methods = [
        ...
        `Le traitement à ${city} débute par un repérage des zones d'activité des rongeurs. G&F Nuisibles installe ensuite des dispositifs professionnels adaptés et procède au colmatage des points d'entrée. Un contrôle post-intervention est programmé pour valider l'élimination complète en ${department}.`,
        `À ${city}, G&F Nuisibles réalise un audit complet de vos locaux : traces de rongement, déjections, empreintes. Nous déployons un dispositif de dératisation sur mesure en ${department}, avec suivi bimensuel et rapport d'intervention détaillé pour les professionnels soumis aux normes HACCP.`,
      ];
  ...
  /* ═══ DÉPIGEONNISATION ═══ */
  {
    ...
    getContent: (city: string, department: string, variation = 0) => {
      ...
      const expertises = [
        ...
        `L'expertise de G&F Nuisibles en ${department} nous permet d'équiper tous types de structures à ${city} : immeubles, entrepôts, monuments, bâtiments industriels. Nos installations anti-pigeons sont quasi invisibles et garantissent une protection longue durée sans nuire aux oiseaux.`,
      ...
      const methods = [
        ...
        `Le traitement à ${city} se déroule en quatre phases : inspection des façades et toitures, choix des dispositifs adaptés, pose professionnelle et désinfection des zones contaminées. G&F Nuisibles fournit un rapport d'intervention détaillé aux gestionnaires de bâtiments en ${department}.`,
        `À ${city}, G&F Nuisibles commence par une étude de site complète. Nous installons ensuite les dispositifs anti-pigeons adaptés à votre architecture en ${department} : pics, filets, systèmes électriques basse tension. Un nettoyage approfondi et une désinfection sont réalisés avant la remise du site.`,
      ];
  ...
  /* ═══ ÉLIMINATION PIGEONS ═══ */
  {
    ...
    getContent: (city: string, department: string, variation = 0) => {
      ...
      const expertises = [
        ...
        `L'expertise de G&F Nuisibles en ${department} nous permet d'intervenir à ${city} sur tous types d'habitations. Nous installons des protections anti-pigeons quasi invisibles qui respectent l'esthétique de votre logement tout en assurant une protection maximale contre les volatiles indésirables.`,
      ...
      const methods = [
        ...
        `Notre protocole chez les particuliers à ${city} comprend : visite d'évaluation, identification des accès et zones de repos, installation de protections sur mesure, puis nettoyage et désinfection. G&F Nuisibles assure un suivi gratuit après l'intervention en ${department}.`,
        ...
        `À ${city}, G&F Nuisibles commence par un diagnostic personnalisé de votre habitation en ${department}. Nous déterminons les points d'entrée et de perchage, installons les protections nécessaires et procédons à un nettoyage complet. Notre suivi post-intervention garantit l'efficacité durable de nos solutions.`,
      ];
  ...
  /* ═══ TAUPES ═══ */
  {
    ...
    getContent: (city: string, department: string, variation = 0) => {
      ...
      const intros = [
        `Votre pelouse à ${city} est ravagée par les taupinières ? G&F Nuisibles, spécialiste du piégeage de taupes en ${department}, intervient rapidement pour protéger vos espaces verts. Les taupes, bien qu'utiles pour l'aération du sol, peuvent causer des dégâts considérables sur les jardins, terrains de sport et espaces paysagers à ${city}.`,
        `Les taupes envahissent votre jardin à ${city} et multiplient les monticules de terre ? G&F Nuisibles en ${department} utilise la méthode traditionnelle de piégeage, la plus efficace et la plus respectueuse de l'environnement. Préservez vos espaces verts avec notre expertise terrain reconnue à ${city} et ses alentours.`,
        `G&F Nuisibles à ${city} met son savoir-faire au service de l'élimination des taupes en ${department}. Nos techniciens utilisent exclusivement des pièges mécaniques professionnels, sans aucun produit chimique. Cette méthode écologique garantit des résultats rapides tout en respectant l'écosystème de votre jardin à ${city}.`,
        `À ${city}, en ${department}, les taupes peuvent transformer un beau jardin en champ de bataille. G&F Nuisibles vous apporte une solution professionnelle et écologique : le piégeage mécanique traditionnel. Nos experts identifient les galeries actives à ${city} et posent des dispositifs ciblés pour une élimination efficace.`,
      ];

      const expertises = [
        `À ${city} en ${department}, nos techniciens identifient les galeries actives et posent des pièges mécaniques professionnels. Aucun produit chimique n'est utilisé. Notre expertise terrain garantit des résultats rapides et durables. G&F Nuisibles intervient chez les particuliers, sur les terrains sportifs, golfs et espaces verts municipaux.`,
        `G&F Nuisibles déploie à ${city} une méthode de piégeage éprouvée en ${department}. Nos techniciens analysent le réseau de galeries, identifient les passages principaux et positionnent les pièges avec précision. Cette approche ciblée assure un taux de capture élevé sans impact environnemental.`,
        `L'expertise de G&F Nuisibles en ${department} repose sur une connaissance approfondie du comportement des taupes. À ${city}, nous intervenons sur tous types de terrains : jardins privés, parcs, terrains de sport, golfs et espaces communaux. Nos méthodes 100 % mécaniques respectent la faune et la flore environnantes.`,
      ...
      const methods = [
        ...
        `Le traitement de votre terrain à ${city} se déroule sur plusieurs passages : repérage initial, pose de pièges mécaniques sélectifs, contrôle régulier et ajustement des positions si nécessaire. G&F Nuisibles garantit des résultats concrets et mesurables en ${department}.`,
        `À ${city}, G&F Nuisibles réalise un diagnostic terrain complet : cartographie des taupinières, sondage des galeries, identification des zones d'activité. Nous déployons ensuite un dispositif de pièges mécaniques en ${department}, avec suivi régulier jusqu'à la résolution complète du problème.`,
      ];
...
export const ALL_SERVICES = ["guepes-frelons", "desinsectisation", "deratisation", "depigeonnisation", "elimination-pigeons", "taupes"];
